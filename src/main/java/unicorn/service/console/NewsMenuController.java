...
public class NewsMenuController {
    ...
    private void handleSearchByType() {
        try {
            ...
            if (type == null) {
                throw NewsException.invalidType();
            }
            ...
            if (newsList == null) {
                throw new NewsException("Error al recuperar comunicados");
            }
            ...
            showError("Error al buscar por tipo: " + e.getMessage());
        } catch (IllegalArgumentException e) {
            showError("Tipo de notificación no válido: " + e.getMessage());
        }
    }
    ...
    private TipoNews readType() throws NewsException {
        System.out.println("\nTipos disponibles:");
        ...
        while (true) {
            try {
                ...
                TipoNews type = TipoNews.valueOf(input.toUpperCase());

                // Validación adicional del tipo
                if (type == null) {
                    throw NewsException.invalidType();
                }
                return type;
    ...
    private int readIntOption(String prompt) {
        while (true) {
            try {
                System.out.print(prompt);
